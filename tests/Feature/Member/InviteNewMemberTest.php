...
test('can invite a new member', function () {
    ...
    $response = $this->actingAs($user)->post(route('inviteMember'), [
...
test('can resend pending invitation', function () {
    ...
    $response = $this->actingAs($user)->post(route('resendInvitation', $invitation));
...
test('can revoke pending invitation', function () {
    ...
    $response = $this->actingAs($user)->delete(route('revokeInvitation', $invitation));
...
test('cannot access invitation from different workspace', function () {
    ...
    $response = $this->actingAs($user1)->post(route('resendInvitation', $invitation));
    $response->assertForbidden();

    $response = $this->actingAs($user1)->delete(route('revokeInvitation', $invitation));
    $response->assertForbidden();
});
...
test('guests cannot invite members', function () {
    $response = $this->post(route('inviteMember'), [
        'email' => 'test@example.com',
        'role' => 'Member',
    ]);
